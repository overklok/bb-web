...
import { DomainDecl, DomainSlice } from "../core/extras/types";
...
export default class BackgroundLayer extends Layer {
    ...
    private _drawDomains() {
        ...
        for (const d of Object.values(this.__grid.domains)) {
            ...
            const d_from = this.__grid.getCell(from.x, from.y, BorderType.Wrap).idx,
    ...
    private _drawDomain(
        ...
        if (this.__schematic) {
            ...
            if (after > 0) {
                const cell_from_add = this.__grid.getCell(cell_to.idx.x, cell_to.idx.y);
                const cell_to_add = this.__grid.getCell(
                    cell_to.idx.x + after * Number(is_horizontal),
                    cell_to.idx.y + after * Number(is_vertical)
                );

                this._drawDomainLine(container, cell_from_add, cell_to_add, inversed, false, color, dotted);
            }
            ...
            if (before > 0) {
                ...
                const cell_to_add = this.__grid.getCell(cell_from.idx.x, cell_from.idx.y);
                this._drawDomainLine(container, cell_from_add, cell_to_add, inversed, false, color, dotted);
            }
        } else {
     ...
     */
    private _drawDomainRect(container: SVG.Container, cell_from: Cell, cell_to: Cell, color: string) {
