...
export type PlateParams = {
    ...
    adjs: { [orientation: string]: XYPoint };
...
export default abstract class Plate {
    ...
    constructor(
        ...
        if ("onwheel" in document) {
            ...
            this._group.node.addEventListener("mousewheel", (evt) =>
                this._callbacks.mousewheel(evt)
            );
        } else {
            // Firefox < 17
            this._group.node.addEventListener("MozMousePixelScroll", (evt) =>
                this._callbacks.mousewheel(evt)
            );
        }
    }
    ...
    public draw(cell: Cell, orientation: string, animate: boolean = false) {
        ...
        let width =
            cell.size.x * this._params.size.x +
            this.__grid.gap.x * 2 * (this._params.size.x - 1);
        let height =
            cell.size.y * this._params.size.y +
            this.__grid.gap.y * 2 * (this._params.size.y - 1);
    ...
    public shift(dx: number, dy: number, prevent_overflow: boolean = true) {
        this.move(
            this.__grid.getCell(
                this._state.cell.idx.x + dx,
                this._state.cell.idx.y + dy,
    ...
    public rotate(
        ...
        let anchor_point = {
            x:
                this._params.origin.x * (this.__grid.gap.x * 2 + cell.size.x) +
                cell.size.x / 2,
            y:
                this._params.origin.y * (this.__grid.gap.y * 2 + cell.size.y) +
                cell.size.y / 2
        };
    ...
    private _checkParams() {
        if (
            this._params.origin.x >= this._params.size.x ||
            this._params.origin.y >= this._params.size.y
        ) {
    ...
    public calcSupposedCell() {
        ...
        let [Ox, Oy, Nx, Ny] = this._getPlacementConstraints(
        ...
        /// Соседи по краям
        if (ix + 1 <= Nx) neighbors.push(this.__grid.getCell(ix + 1, iy));
        if (ix - 1 >= Ox) neighbors.push(this.__grid.getCell(ix - 1, iy));
        if (iy + 1 <= Ny) neighbors.push(this.__grid.getCell(ix, iy + 1));
        if (iy - 1 >= Oy) neighbors.push(this.__grid.getCell(ix, iy - 1));
     ...
     */
    private _calcPlacementConstraints(): {
        [orientation: string]: [number, number, number, number];
    } {
        /// Размерность доски
    ...
    private _reoccupyCells(clear: boolean = false) {
        ...
        for (let _rel of this._params.rels) {
            ...
            let adj_cur = clear
                ? null
                : Plate._orientXYObject(_rel.adj, this._state.orientation);
            // определить корректировку положения всей плашки
            let adj_pos = this._params.adjs
                ? this._params.adjs[this._state.orientation]
                : null;
     ...
     */
    private _getPositionAdjusted(cell: Cell = null): XYPoint {
    ...
    private _generateSurfacePath(radius = 5): (string | number)[][] {
        if (this._params.surface) {
            ...
            let cell = this.__grid.getCell(surf_point.x, surf_point.y);
    ...
    private _buildSurfacePath(
        ...
        while (surfcnt[cell.idx.x][cell.idx.y] < dirs.length) {
            ...
            if (
                ...
                path = path.concat(
                    this._buildSurfacePathEdge(cell, dir, radius)
                );
            }
    ...
    private _convertSurfaceToArray(
        ...
        for (let item of surface) {
            ...
            if (arr[item.x].length > this._params.size.y) {
                console.error(
                    "Invalid surface for Y size, skipping custom bezel"
                );
                return;
            }
            ...
            if (arr.length > this._params.size.x) {
                console.error(
                    "Invalid surface for X size, skipping custom bezel"
                );
                return;
            }
        }
    ...
    private static _orientationToAngle(orientation: string) {
        switch (orientation) {
            ...
            default: {
                throw new TypeError(
                    `Invalid 'orientation' argument: ${orientation}`
                );
            }
        }
    }
