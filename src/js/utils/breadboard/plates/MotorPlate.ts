...
export default class MotorPlate extends LinearPlate {
    ...
    private _drawPicture(qs = Plate.QuadSizePreferred) {
        ...
        this._group
            .line(center_x - 10, center_y + 7, center_x + 10, center_y + 7)
            .stroke({ width: 2 })
            .fill("none");

        this._group
            .line(center_x - 10, center_y + 12, center_x - 2, center_y + 12)
            .stroke({ width: 2 })
            .fill("none");

        this._group
            .line(center_x + 2, center_y + 12, center_x + 10, center_y + 12)
            .stroke({ width: 2 })
            .fill("none");
    }

    private _drawLabel(text = "", size = Plate.LabelFontSizePreferred) {
        let label = this._group
            .text(String(text))
            .font({
                size: size,
