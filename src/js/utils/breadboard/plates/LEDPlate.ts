...
export default class LEDPlate extends LinearPlate {
    ...
    protected __setProps__(props: PlateProps) {
        ...
        if (colour === "R") {
            colour = LEDPlate.COLOURS.RED;
        }
        if (colour === "G") {
            colour = LEDPlate.COLOURS.GREEN;
        }
        if (colour === "B") {
            colour = LEDPlate.COLOURS.BLUE;
        }
        ...
        if (colour !== 0 && colour !== 1 && colour !== 2) {
            colour = 0;
            console.error(
                "Colour of LED must be one of R, G, B, 0, 1, 2. Fall back to 0"
            );
        }
    ...
    private _redrawOutput(output_value: string) {
        if (!this._svgout) {
            let cell = this.__grid.getCell(0, 0);
    ...
    private _drawPicture(qs = Plate.QuadSizePreferred) {
        ...
        let ptr1 = this._group
            .polyline(ptrpath)
            .stroke({ width: 1 })
            .fill("#000");
        let ptr2 = this._group
            .polyline(ptrpath)
            .stroke({ width: 1 })
            .fill("#000");
    ...
    private _shortLabel(): string {
        switch (this._props[LEDPlate.PROP_COLOUR]) {
            case LEDPlate.COLOURS.RED:
                return "R";
                break;
            case LEDPlate.COLOURS.GREEN:
                return "G";
                break;
            case LEDPlate.COLOURS.BLUE:
                return "B";
                break;
        }
